...
impl Db {
    ...
    pub fn insert_vault<S: AsRef<OsStr> + ?Sized>(&self, path: &S, parent_vault_id: i64) -> Result<i64, String> {
        let mut stmt = self.0.prepare_cached(V_UPSERT_OR_KEEP).unwrap();
        let res = stmt.insert([path.as_ref().to_str().unwrap(), &parent_vault_id.to_string()]);
        match res {
            Ok(i) => Ok(i),
            Err(e) => Err(e.to_string())
        }
    ...
    pub fn insert_project<S: AsRef<OsStr> + ?Sized>(&self, name: &S, modified: &str, vault_id: i64) -> Result<i64, String> {
        let mut stmt = self.0.prepare_cached(V_UPSERT_OR_KEEP).unwrap();
        let res = stmt.insert([name.as_ref().to_str().unwrap(), modified, &vault_id.to_string()]);
        match res {
            Ok(i) => Ok(i),
            Err(e) => Err(e.to_string())
        }
...
const V_SET_KEEP: &str = 
"UPDATE Vaults SET Keep = (?1)";
const V_PURGE_KEEP: &str =
"DELETE FROM Vaults WHERE Keep = 0";
const V_GET_ALL_BY_ID: &str =
"SELECT * FROM Vaults WHERE VaultId = (?)";
const V_GET_ALL_BY_PATH: &str =
"SELECT VaultId from Vaults WHERE Path=(?)";
const V_UPSERT_OR_KEEP: &str =
"INSERT OR IGNORE INTO Vaults (Path, ParentVaultId) values (?1, ?2)
    ON CONFLICT DO UPDATE set Keep = 1";

const P_CREATE_TABLE: &str = 
...
const P_SET_KEEP: &str = 
"UPDATE Projects SET Keep = (?1)";
const P_PURGE_KEEP: &str =
"DELETE FROM Projects WHERE Keep = 0";
const P_UPDATE_TIMESTAMP: &str =
"UPDATE Projects set Timestamp = CURRENT_TIMESTAMP WHERE ProjectId = (?1)";
const P_GET_ALL_BY_ID: &str =
"SELECT * FROM Projects WHERE ProjectId = (?1)";
const p_UPSERT_OR_KEEP: &str =
"INSERT INTO Projects (Name, Modified, VaultId) values (?1, ?2, ?3)
